...
db.serialize(() => {
    ...
    db.run(`
        CREATE TABLE IF NOT EXISTS store_settings (
            ...
            weight_breakfast REAL DEFAULT 0.89,
            weight_lunch REAL DEFAULT 1.17,
            weight_afternoon REAL DEFAULT 1.04,
            weight_dinner REAL DEFAULT 0.90,
            updated_at DATETIME DEFAULT CURRENT_TIMESTAMP
        )
    `);
...
app.put('/api/store/:storeName/settings', (req, res) => {
    ...
    db.run(query, [
        storeName,
        ambition_tier,
        weights.breakfast ?? 0.89,
        weights.lunch ?? 1.17,
        weights.afternoon ?? 1.04,
        weights.dinner ?? 0.90
    ], function(err) {
        if (err) {
            return res.status(500).json({ error: err.message });
