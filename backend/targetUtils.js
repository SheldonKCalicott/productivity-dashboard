...
function getAmbitionMultiplier(selectedTier = 'Balanced') {
...
function calculateForecastFromHistoryRecords(records = [], referenceDate, options = {}) {
  ...
  const weekdayDays = completeDays.filter((day) => day.weekday === targetWeekday).slice(0, weekdaySampleSize);
  ...
  if (selectedDays.length === 0) {
    return {
      ...
      sampleDates: [],
      completedOperationalDays: completeDays.length,
    };
  }
...
function calculateProjectedDailySales(daypartSales = {}, historicalAverages = DEFAULT_DAYPART_AVERAGES, options = {}) {
...
function calculateAdaptiveLearningProfile({ records = [], previousProfile = null, selectedTier = 'Balanced', closedWeekdays = [] } = {}) {
...
function calculateTargetProductivity(daypartKey, salesAmount, selectedTier = 'Balanced', daypartWeights = DEFAULT_OPERATIONAL_WEIGHTS) {
