...
export default function DaypartDashboard({ onNavigateToReports }) {
    ...
    const calculateTarget = (sales, salesRange, productivityRange, daypartKey, totalSales) => {
        if (!sales || sales === '' || isNaN(Number(sales))) return 0
        const numSales = Number(sales)
        
        // Use tier-based calculation if total sales provided, otherwise use original range-based
        if (totalSales && daypartKey) {
            return calculateDaypartTarget(numSales, totalSales, daypartKey)
        }
    ...
    const getDayCombinedTarget = () => {
        const bfSales = getDaypartSales('breakfast') || 6000
        const lnSales = getDaypartSales('lunch') || 10000
    ...
    const getNightCombinedTarget = () => {
        const afSales = getDaypartSales('afternoon') || 7000
        const dnSales = getDaypartSales('dinner') || 8000
